...
In this paper, I study how electoral campaigns shape the demand for partisan news and, in turn, political polarization. I construct a daily, story-level measure of slant in Spanish TV news using machine learning and Large Language Models (LLMs) and link it to high-frequency audience-meter records. I then develop and estimate a structural model of news demand and supply. Leveraging exogenous news supply shocks as cost shifters to identify demand, I show that polarization in news consumption intensifies only when the electoral campaign begins, with out-party animosity dominating. Regions with more polarized media consumption also exhibit a sharper and more persistent rise in ideological polarization. Finally, I evaluate the effects of a media regulation policy that enforces proportional-airtime rule during campaigns. The regulation has unexpected effects: it widens cross-outlet dispersion in supplied slant, thereby amplifying audience responses to partisan content, yielding a more polarized media landscape.
...
%This paper investigates whether and how political polarization during campaigns is linked to media consumption and evaluates the effect of regulation aimed at minimizing it.
...
Most of the heterogeneity is captured by the demographics (i.e ideology).  The estimated taste-dispersion parameters (\(\sigma\)s) are small and very imprecise. The high standard errors on these coefficients arise by construction. The identification of the random-coefficients  relies on variation across markets in the choice set, which is null in my case given that the same products are present across regions and dates. \footnote{A similar result was documented by \citet{Nevo1998MeasuringMP}. Consistent with his findings, the relative importance of demographics versus random shocks—computed as the ratio of variance explained by demographics to the total variance in the distribution of estimated coefficients—exceeds 90\% in my model.} These low values of heterogeneity naturally link to the similar results that I obtain under the multinomial logit specification shown in Table \ref{tab:logit}. 
...
\begin{table}[!htbp]
	...
	\begin{tabular}{lcccccc}
		...
		& Baseline & Count. & Baseline & Count. & Baselina & Count. \\
		\midrule
...
This paper studies how election campaigns reshape demand for political news using a structural model of television news supply and demand. I construct a story-level slant index for prime-time broadcasts by combining machine learning with large language models (LLMs), and match it to high-frequency audience data. This allows me to track the slant content offered in the news in parallel to the amount of people consuming it. 


I apply the framework to Spain’s 2023 general election. On the demand side, I estimate a BLP-style random-coefficients model identified by exogenous variation in  set of stories available to produce slant.  The results show that political news consumption polarizes sharply during the campaign and the information pattern is consistent with affective polarization: viewers dislike negative news about their own party but they partically screen out any content that benefit the opposing party. Furthermore, I show that polarization in demand for political content links tightly to ideological polarization, where election campaigns triggered a surge in the latter that lasts for over a year. 
 


Building on the demand and supply estimates, I evaluate the most common media regulation to foster pluralism during campaigns: proportional-airtime rules. The counterfactual results show that, while channels offer a balanced coverage of the political parties, this obscures increases in the polarization of the slant they offer.  The reason for this stems from the design of the policy: since only total exposure in minutes aired is enforced,  outlets comply by adjusting the tone, widening cross-outlet dispersion in supplied slant and strengthening audience responses to partisan content.


This paper highlights two main points. First, surges in ideological polarization during periods of intense political conflict are closely mirrored by the way voters consume information. Second, widely used metrics for monitoring media bias—such as airtime balance—often overlook crucial differences in political tone, leading to interventions and regulations with unintended consequences. When policy is designed using incomplete or misaligned measures, regulation can inadvertently amplify polarization along dimensions it fails to capture, such as editorial slant. The methodological framework developed here provides a more robust basis for media monitoring and a path for evaluating regulatory measures that  effectively reduce polarization in information acquisition. 


Several avenues for future research emerge from the present work. Leveraging the unique feature of the dataset—which allows to compare the tone and coverage of the same story across outlets on a give date—future work could micro-found editorial decisions  from the underlying news landscape to distinguish between different forms of bias, such as selection, and framing, as theorized in previous works \citep[e.g.][]{gentzkow2014media}. Another direction XXX better understanding the role of newswire agencies as providers of information...




%would be to extend the estimation framework to multi-dimensional ideological spaces and dynamic models of media consumption and supply, incorporating feedback between ideology and media consumption to test the extent of media persuasion.
