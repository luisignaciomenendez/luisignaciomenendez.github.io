...
where $\bm z_{dh}$ measures the pool of stories available at edition~$h$, meaning the stories available up until the beginning of the midday ($h=0$) and evening editions ($h=1$) of the newscasts. $\bm x_{jdh}$ is the proportion of minutes to each tone category as defined in Eq. \ref{eq:controls}. $L_r$  refers to the total potential audience in region $r$. Notice that I have now incorporated the midday dimension in this analysis. The reason for this will become apparent below in the description of the identification strategy. 
...
Given the demand and supply estimates, I recover the total payoffs for each outlet and tone.  Figure \ref{fig:payoffs} plots the payoff functions (in thousands of viewers) \footnote{Figure \ref{fig:payoffs_constrained} shows the payoffs under the non-negativity constraint. The ranking of payoffs is stable across the two specifications.} For both parties, negative-tone categories dominate: the right-leaning channel obtains a maximum of  around 500 thousand viewers with negative-left content (vs. 400 thousand at the maximum positive-left tone), while the left-leaning outlets get a maximum of around 300 thousand with negative-right content (vs. 90 thousand with positive-right). To better illustrate this negativity premium, Figure \ref{fig:payoffs_surface} plots the payoff surface for each channel varying the right and left tones. It becomes apparent that the outlets obtain higher viewership by becoming more negative on the opposing party, with the middle channel reaching a maximum with a balanced coverage. 
